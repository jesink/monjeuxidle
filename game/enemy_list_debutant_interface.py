...
class EnemyListDebutantInterface:
    def __init__(self, screen, character, combat_interface_class):
        ...
        # Créez une liste d'ennemis de niveaux 11 à 20 avec des statistiques adaptées
        self.enemies = [
    ...
    def display(self):
        ...
        while running:
            ...
            # Dessiner le bouton "Retour" en haut à droite
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # Vérifier si un ennemi a été cliqué
                    for index, enemy in enumerate(self.enemies):
                        rect_x = x_start
                        rect_y = y_start + index * y_offset
                        if rect_x <= mouse_x <= rect_x + slot_width and rect_y <= mouse_y <= rect_y + slot_height:
                            selected_enemy = enemy
                            combat_interface = self.combat_interface_class(self.screen, self.character, selected_enemy, self)
                            combat_interface.display()
