...
class EnemyListTresDifficileInterface:
    def __init__(self, screen, character, combat_interface_class):
        self.screen = screen
        self.character = character
        self.combat_interface_class = combat_interface_class  # Ajoutez cette ligne
        self.background_image = pygame.image.load(os.path.join("assets", "background.png")).convert()

        # Créez une liste d'ennemis avec des statistiques adaptées (par exemple, pour les niveaux 60 à 70)
        self.enemies = [
            {"level": level, "stats": self.generate_stats(level)}
            for level in range(60, 71)
        ]
